...
from models.rol import RolCreate, RolUpdate, PermisosAsignacion, PermisoResponse, RolResponse
...
async def get_roles(session: SessionDep, _=Depends(require_admin)):
...
async def get_permisos(session: SessionDep, _=Depends(require_admin)):
...
async def create_rol(rol_data: RolCreate, session: SessionDep, _=Depends(require_admin)):
    try:
...
async def update_rol(rol_id: int, rol_data: RolUpdate, session: SessionDep, _=Depends(require_admin)):
    try:
...
async def asignar_permisos(
    rol_id: int, asignacion: PermisosAsignacion, session: SessionDep, _=Depends(require_admin)
):
    try:
...
async def delete_rol(rol_id: int, session: SessionDep, _=Depends(require_admin)):
    try:
